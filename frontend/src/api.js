...
const request=async(path,options={})=>{const token=localStorage.getItem('vc-token');const headers={...(options.body instanceof FormData?{}:{'Content-Type':'application/json'}),...options.headers};if(token)headers.Authorization='Bearer '+token;const response=await fetch(BASE+path,{...options,headers});const data=await response.json().catch(()=>({}));if(response.status===401&&token){localStorage.removeItem('vc-token');window.dispatchEvent(new Event('vc-session-expired'))}if(!response.ok)throw new Error(data.message||'Request failed');return data};
export const api={
 ...
 recoverPassword:(email,recoveryCode,newPassword)=>request('/auth/recover',{method:'POST',body:JSON.stringify({email,recoveryCode,newPassword})}),
 ...
 reorderMedia:(id,publicIds)=>request('/products/'+id+'/media-order',{method:'PATCH',body:JSON.stringify({publicIds})}),
 deleteMedia:(id,publicId,resourceType)=>request('/products/'+id+'/media',{method:'DELETE',body:JSON.stringify({publicId,resourceType})}),
